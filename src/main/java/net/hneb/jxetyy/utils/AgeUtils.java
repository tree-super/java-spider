...
public class AgeUtils {
	 ...
	 * @return Map<String,Integer> YEAR 年 MONTH 月 DAY 日
	 */
...
//		c2.setTime(date);
		...
		if (day < 0) {
			...
			if (preMonth == 1 || preMonth == 3 || preMonth == 5 || preMonth == 7 || preMonth == 8 || preMonth == 10
					|| preMonth == 12) {
	...
	/**
	 * 功能同calcAge，只是返回值不同而已，参见calcAge
	 * 
	 * @param date
	 * @param referDate
	 * @return
	...
	public static String calcAgeAsString(Date date, Date referDate) {
		Map<String, Integer> map = AgeUtils.calcAge(date, referDate);

		int year = map.get("YEAR");
		int month = map.get("MONTH");
		int day = map.get("DAY");
		StringBuffer result = new StringBuffer();
//		result.append(year>0?(year+"岁"):"");
//		result.append(month>0?(month+"个月"):"");
//		result.append(day+"天");
